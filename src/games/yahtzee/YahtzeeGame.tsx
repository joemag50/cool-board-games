...
export function YahtzeeGame() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-900 flex items-start justify-center px-4 py-6">
      {/* Game over overlay */}
      {game.gameOver && (
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full max-w-4xl">
        ...
        <div className="flex flex-col items-center lg:items-start lg:w-80 shrink-0">
          {/* Dice area */}
          <div className="grid grid-cols-5 lg:grid-cols-3 gap-3 mb-5">
          ...
          <button
            ...
            className={`
              w-full px-8 py-3 rounded-xl font-bold text-lg transition-all mb-3
          ...
          <span className="text-slate-400 text-sm mb-4 text-center w-full">
          ...
          {/* Grand total – visible on left panel too */}
          <div className="hidden lg:flex w-full mt-auto pt-6 justify-between items-center px-4 py-3 bg-gradient-to-r from-amber-900/40 to-amber-800/40 rounded-xl">
            <span className="text-amber-200 font-semibold">TOTAL</span>
            <span className="text-amber-400 font-bold text-2xl">{game.grandTotal}</span>
          </div>
        </div>

        {/* Right panel – Scorecard */}
        <div className="flex-1 bg-slate-800/60 rounded-2xl overflow-hidden shadow-xl border border-slate-700/50">
          {/* Upper section */}
